...
export default {
  ...
  mutations: {
    SET_DATA_SCALE_MBSS(state, scoreHistoryMbss) {
      state.scoreHistoryGraphDataMbss = scoreHistoryMbss;
    },
    SET_PARTS_SELECTED(state, data) {
      state.partsScaleSelectedMbss = data;
    },
  ...
  actions: {
    async getScoreScaleMbss({ commit, rootState }, scaleId, nameScale) {
      ...
      commit("SET_DATA_SCALE_MBSS", {data: scores });
